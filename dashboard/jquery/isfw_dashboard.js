...
function applyUi(container) {
	...
	$(container).find('.meta-info.action').addClass('ui-state-active');
	...
	$(container).find('.check-points.action').bind("click", function(event) {
		toggleTab($(this), '.check-points');
	});
	...
	$(container).find('.meta-info.action').bind("click", function(event) {
...
function drawTrendChart(reports) {
	...
	$.each(lstReports, function(i, report) {
		chained = $.getJSON(report + "/meta-info.json", function(details) {
...
function expandCollapseAll(expand) {
	...
	if ($('.mehod:not(:hidden) .details:empty').length > 0 && expand) {
		$('#progress').html('0%');
		$('#loading-info').show();
	}
...
function doSortE(isReverse) {
	interuptLoading = true;
	$('.mehod').tsort('.mehodheader .oexecution', {
		sortFunction : function(a, b) {
			retVal = a.s == b.s ? 0 : parseInt(a.s) > parseInt(b.s) ? (isReverse)?-1:1 : (isReverse)?1:-1;
			return retVal;
		}
	});
}
